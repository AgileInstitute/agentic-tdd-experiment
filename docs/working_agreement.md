...
- Flow is tracked in `story_map.md` at the repo root (renamed 2026-08-13 from `queue.md`; not yet a positionally-prioritized story map — no release/priority axis yet, just flat themes/stories — revisit once scope selection for a Minimal Lovable Product matters). Format:
  ...
  - Stories numbered within their theme (`### N.M Headline`, e.g. `5.2 Importing photo information`). Numbers are permanent once assigned — never renumbered, even when priority shifts or a new story logically belongs earlier in the sequence. A new story always takes the next unused number in its theme, regardless of where it sits in priority; ordering within a theme is not assumed to match story number order.
...
- **Triangulation cluster cap**: a "cluster" is a batch of tests that tightly triangulate the same behavior (e.g. TextRepairer's good text / bad text / good European text / multiple bad-byte cases). Rob always pauses to review tests before any of them get implemented (a hard exception to the auto-mode default of not pausing for per-step confirmation) — the cap doesn't govern *whether* to pause, only how clusters get *grouped* into a pause point. This pause is non-waivable: general "proceed"/"just get it done"/"whatever you need to do to complete the story" instructions do not skip it — only an instruction that unambiguously addresses this specific pause does. The cap is currently a dozen specs:
  - Clusters at or under the cap can be grouped together: Claude can write several of them, show them all in one pass, and after a single pause implement everything just shown, uninterrupted.
  - A cluster over the cap always stands alone — never bundled with other clusters into the same pass. It's shown by itself, and once Rob gives the go-ahead, is implemented and committed on its own before moving to the next cluster.
  - Multiple clusters shown together in one pass are separated by clear whitespace so each still reads as distinct.
...
- **Display cadence** per review-and-implement pass (worth following closely since sessions may be recorded for demonstration purposes, but it's the default either way):
  1. Post the to-do list with the next item(s) marked (`->`/`<-` and/or bold).
  2. Show the new spec(s) as written — one or more at-or-under-cap clusters grouped together, or a single over-cap cluster shown alone (see cap above).
  3. Show the RSpec failure output (red), then stop and wait for Rob's go-ahead before continuing to step 4.
  4. Show the implementation as a diff for everything just reviewed, the same way any other proposed change is shown. Narration can describe what the code does, but the audience is non-technical, so keep it accessible rather than jargon-heavy.
...
- When a story's refactor touches an existing spec, only its use of implementation interfaces and objects may change to reflect implementation growth — its intent (the guarantee it verifies) must still hold. If a story deliberately reverses a prior spec's intent, that's not a refactor touch-up — call it out explicitly as retiring/superseding that spec. Specs that no longer make sense can just be deleted; Rob reviews the deletion in the diff like any other change.
